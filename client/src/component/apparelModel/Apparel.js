...
export default function Apparel(props) {
  ...
  const MODEL_ID = "apparel-recognition";
  ...
  const ApparelList = (data) => {
    const clarifaiList = data.outputs[0].data.concepts;
    return clarifaiList;
  };

  const onApparelSubmit = () => {
    setProcessing(true);
    fetch(
      ...
      .then((result) => {
        ...
        if (result.outputs[0].data.concepts.length >= 1) {
          fetch("https://aiworldbyjemil.herokuapp.com/image/apparelmodel", {
            method: "put",
            headers: { "content-Type": "application/json" },
            body: JSON.stringify({
              id: props.userdata.id,
              apparelEntries: props.userdata.apparelEntries,
            }),
          })
            .then((response) => response.json())
            .then((data) => {
              if (data) {
                props.setUser((prev) => {
                  return {
                    ...prev,
                    apparelEntries: data.apparelEntries,
                  };
                });
                props.setApparel(ApparelList(result));
              }
            })
            .catch(() => {
              setProcessing(false);
  ...
  const apparelMappedItems = props.apparel.map((obj, i) => {
    return (
    ...
    <>
      ...
      <div className="center">
        ...
        <ImageLinkForm
          ...
          onApparelSubmit={onApparelSubmit}
          route={props.route}
          processing={processing}
        />
        <div className="center">
          <Image inputState={props.inputState} />
          {props.apparel.length >= 1 && props.inputState.length > 10 && (
            <div className="result-wrapper">
              <div className="FaTimesCircle">
                <FaTimesCircle
                  size="1.7em"
                  color="white"
                  onClick={clearApparel}
                />
              </div>
              <div style={{ color: "white" }}>
                Number of detections: {props.apparel.length}
              </div>
